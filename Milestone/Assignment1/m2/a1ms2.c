...
int main(void)
{
	...
	struct Contact contact = {{{ 0 }}}; //initializing by 0.
	...
	getName(&contact.name); //Function call for getting name

	// Call the Contact function getAddress to store the values for the Address member
	getAddress(&contact.address); //Function call for getting address

	// Call the Contact function getNumbers to store the values for the Numbers member
	getNumbers(&contact.numbers); //Function call for getting numbers

	// Display Contact summary details
	printf("\nContact Details");
	printf("\n---------------\n");
	printf("Name Details\n");
	printf("First name: %s", contact.name.firstName); //take value through contact -> structure which is contained in contact structure -> address in structure in contact structure.
